...
from .ppl_loss import compute_ppl_loss
...
class CustomSeq2SeqPPLTrainer(Seq2SeqTrainer):
    ...
    def concatenated_forward(self, model, batch):
        ...
        all_logps, _ = get_batch_logps(logits=all_logits, labels=batch["labels"])

        pos_logps, neg_logps = all_logps[:1], all_logps[1:]
        return pos_logps, neg_logps, outputs
    ...
    def compute_loss(self, model, inputs, num_items_in_batch=None, return_outputs=False, eval_mode=False):
        ...
        pos_logps, neg_logps, outputs = self.concatenated_forward(model, inputs)
        
        loss, posterior_loss, llk_loss = compute_ppl_loss(pos_logps, neg_logps)
        print("Positive logps: ", pos_logps)
        print("Negative logps: ", neg_logps)
        print("Posterior loss: ", posterior_loss)
        print("LLK loss: ", llk_loss)
