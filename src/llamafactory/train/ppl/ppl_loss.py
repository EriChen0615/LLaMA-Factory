...
def compute_ensemble_loss(logits, labels, prior_logits=None, per_token_logps=None):
    ...
    if prior_logits is not None:
        ...
        log_passage_prior = torch.zeros((logits.shape[0], 1), device=device) # shape (K, 1)
    logm = log_passage_prior

    token_logps, cumulative_token_logps, valid_length = _get_token_and_cumulative_logps(logits, labels, per_token_logps=per_token_logps) # shape (K, N), where N is the number of answer tokens
    K, N = token_logps.shape[0], valid_length

    logp = token_logps # shape (K, N)
    logq = torch.cat([torch.zeros((K, 1), device=device), cumulative_token_logps[:,:N-1]], dim=-1) # shape (K, N) #NOTE log(q0)=0 for all K. i.e., uniform prior. 
